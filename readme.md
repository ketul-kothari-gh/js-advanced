...
JS engine creates EC that manages the code (in lexial env) that is currently running. There can be multilpe execution contexts: - Global execution context - Function execution context.
...
**_ Special case _** If a function is created inside another function attached to object, this points to globalObject.
...
**_ Closures are not supported in a lot of OOPS programming languages but is common in functional programming languges _**
...
JS module is just a script.
Export is used to ensure that another module can access vairable, functions defined in source module.
Import is used to import exported variables, functions in the required module.

Modules are special features, to specify that script is going to import module - must use type="module" in script tag.
Modules are by default deferred by the browser i.e. loaded once that html has loaded. (same as using defer in script tag).
Modules always work in script mode.

# spread and rest

The spread operator ( ... ) copies all or part of an existing array or object into another array or object.
